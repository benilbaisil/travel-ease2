...
/* 
    ...
    <body>
        ...
        function generateUserBookingsPDF(userData) {
            ...
            doc.text(`User Report: ${userData.name}`, 105, 20, { align: "center" });
            ...
            doc.text("Booking Information", 20, doc.lastAutoTable.finalY + 15);
            
            if (userData.bookings.length === 0) {
                doc.setFontSize(12);
                doc.setTextColor(150, 150, 150);
                doc.text("No bookings found for this user", 20, doc.lastAutoTable.finalY + 30);
            } else {
                ...
                const bookingSummary = [
                    ...
                    ["Total Spent (Confirmed)", `₹${totalSpent.toFixed(2)}`]
                ];
                ...
                doc.autoTable({
                    startY: doc.lastAutoTable.finalY + 20,
                    head: [["ID", "Package", "Date", "Price", "Status"]],
                    body: bookingDetails,
                    theme: 'grid',
                    headStyles: { 
                        fillColor: [231, 76, 60]
            ...
            // Save the PDF
            doc.save(`user-report-${userData.name.replace(/\s+/g, '-')}-${date}.pdf`);
        }
        ...
        function exportReportPDF() {
            ...
            doc.text("Travel Ease System Report", 105, 20, { align: "center" });
            doc.setFontSize(12);
            ...
            const revenueStats = [
                ["Total Revenue", "₹<?php echo number_format($stats['total_revenue'], 2); ?>"],
                ["Average Booking Value", "₹<?php echo $stats['confirmed_bookings'] > 0 ? number_format($stats['total_revenue'] / $stats['confirmed_bookings'], 2) : '0.00'; ?>"]
            ];
            doc.autoTable({
                startY: doc.lastAutoTable.finalY + 20,
                head: [["Metric", "Value"]],
            ...
            // Save the PDF
            doc.save(`travel-ease-report-${date}.pdf`);
        }
        ...
        function exportPackagesByDateRange() {
            ...
            fetch(`get_packages_by_date.php?start=${startDate}&end=${endDate}`)
                ...
                .then(data => {
                    ...
                    generateDateRangeReport(data, startDate, endDate);
                })
                .catch(error => {
        ...
        function generateDateRangeReport(data, startDate, endDate) {
            const { jsPDF } = window.jspdf;
            const doc = new jsPDF();
            const date = new Date().toLocaleDateString().replace(/\//g, '-');
            ...
            doc.setFontSize(16);
            doc.setTextColor(0, 0, 0);
            doc.text("Package Summary for Selected Period", 20, 45);
            
            const packageSummary = [
                ["Total Packages Booked", data.totalPackages],
                ["Total Bookings", data.totalBookings],
                ["Total Revenue", `₹${data.totalRevenue.toFixed(2)}`],
                ["Average Package Price", `₹${data.avgPrice.toFixed(2)}`],
            ...
            const packageDetails = data.packages.map(package => [
                package.package_name,
                `₹${parseFloat(package.price).toFixed(2)}`,
                package.duration,
                package.booking_count,
            ...
            if (data.bookings && data.bookings.length > 0) {
                // Add a new page for user bookings if we're running out of space
                ...
                doc.setFontSize(16);
                doc.setTextColor(0, 0, 0);
                doc.text("User Booking Details", 20, doc.lastAutoTable.finalY + 15);
                ...
                doc.autoTable({
                    startY: doc.lastAutoTable.finalY + 20,
                    head: [["User Name", "Email", "Package", "Date", "Status", "Price"]],
                    body: bookingDetails,
                    theme: 'grid',
                    headStyles: { 
                        fillColor: [50, 50, 50],
                ...
                doc.setFontSize(12);
                doc.text("No bookings found for the selected date range", 20, doc.lastAutoTable.finalY + 40);
            }
            ...
            // Save the PDF
            doc.save(`travel-ease-packages-report-${startDate}-to-${endDate}.pdf`);
        }
